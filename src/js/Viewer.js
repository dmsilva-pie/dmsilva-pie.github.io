...
function createScene() {
    ...
    camera = new THREE.PerspectiveCamera(30, container.innerWidth / container.innerHeight, 1, 1000);
    camera.up.set(0, 1, 0);
    ...
    helper = new THREE.GridHelper(260, 260, 0xFF4444, 0x404040);
    ...
    renderer1.setSize(container.innerWidth, container.innerHeight);
...
function uploadObj(objFile, params) {
        ...
        loader.load(objFile, function (object) {
            ...
            object.traverse(function (child) {
                if (child instanceof THREE.Mesh) {
                    ...
                    var its = params._this.createSliders(model.geometry.groups.length);
                    if (model.geometry.getAttribute('uv') && model.geometry.groups.length > 0) {
                        var geoParams = {
                            scaleValue: params.scaleValue, textureBounds: params.textureBounds,
                            numberFaces: model.geometry.groups.length, isUpdate: params.isUpdate,
                            wireframeView: params.wireframeView,
                            textureView: params.textureView, numberite: its, mapping: params.mapping
                        };
                        createGeometry(geoParams);
        ...
        loader.load(params.modelSelected, function (object) {
            ...
            object.traverse(function (child) {
                if (child instanceof THREE.Mesh) {
                    ...
                    child.geometry.computeBoundingBox();
                    model = child;
                    var its = params._this.createSliders(model.geometry.groups.length, params.iterations);
                    var geoParams = {
                        scaleValue: params.scaleValue, textureBounds: params.textureBounds,
                        numberFaces: model.geometry.groups.length, isUpdate: params.isUpdate, wireframeView: params.wireframeView,
                        textureView: params.textureView, numberite: its, mapping: params.mapping, color: params.color
                    };
                    createGeometry(geoParams);
                    resolve(model.geometry.groups.length);
                }
...
function createGeometry(params) {
    ...
    if (!params.isUpdate) {
        ...
        groups = getFaces(vertices, model.geometry.groups, normals, params.numberFaces, uvs, params.numberite);
        repositionVertex();
	...
	console.log("after mats\n");


    /** Geometry */
    var geometry = new THREE.Geometry();
    geometry.vertices = texturesImage.length > 0 ? applyTexture(params.scaleValue) : old_vertices;
    var pos = 0;
    for (var i = 0; i < groups.length; i++) {
        for (var f = 0; f < groups[i].faces.length; f++) {
            old_faces[pos + f].materialIndex = params.textureView ? (params.mapping[i] ? params.mapping[i].index + 1 : 0) : 0;
        }
...
function getFaces(vertexs, groups, normals, numberfaces, uvss, iterations) {
    ...
    for (var g = 0; g < numberfaces; g++) {
        ...
        //Sub Division
        if (iterations[g]) {
            loopDivision(group_aux[g], iterations[g].value);
        }
...
function applyTexture(scale) {
    ...
    texturesImage.forEach(texture => {
        ...
        for (var i = 0; i < group.vertices_face.length; i++) {
            ...
            vertices_clone[pos + i].x += old_normals[pos + i].x * d * scale;
            vertices_clone[pos + i].y += old_normals[pos + i].y * d * scale;
            vertices_clone[pos + i].z += old_normals[pos + i].z * d * scale;
        }
    });
...
function loopDivision(group, iterations) {
    ...
    if (iterations === 0) {
        ...
        while (count_divisions < iterations) {
			console.log("Division "+count_divisions+"start!\n");
            //Generate new vertices and uvs
            var new_vertexs = [];
			var new_vertexs_map = {};
            var new_faces = [];
            var new_uvs = [];
			...
			console.log("Division "+count_divisions+"end!\n");
            count_divisions++;
        }
    }
}
...
function checkVertexs(vertices, new_vertices) {
    ...
    /*for (var i = 0; i < vertices.length; i++) {
        for (var j = 0; j < new_vertices.length; j++) {
            if (new_vertices[j][1] === -1 && vertices[i].x === new_vertices[j][0].x && vertices[i].y === new_vertices[j][0].y && vertices[i].z === new_vertices[j][0].z) {
                new_vertices[j][1] = i;
                break;
            }
        }
